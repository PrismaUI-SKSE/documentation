...
- **Note:** Only accepts string data - use a delimiter for complex data structures
...
<TabItem label="Delimited Data Transfer">

```cpp
// Send complex data as a delimited string
void SendPlayerStats() {
    std::string stats = std::to_string(GetPlayerHealth()) + "|"
                      + std::to_string(GetPlayerMagicka()) + "|"
                      + std::to_string(GetPlayerStamina()) + "|"
                      + std::to_string(GetPlayerLevel());

    // Send via InteropCall — no JSON overhead
    PrismaUI->InteropCall(view, "updatePlayerStats", stats.c_str());
}
```

```js
// JavaScript — split by delimiter (faster than JSON.parse)
window.updatePlayerStats = (data) => {
    const [health, magicka, stamina, level] = data.split('|');

    // Update UI elements
    document.getElementById('health').textContent = health;
    document.getElementById('magicka').textContent = magicka;
...
`InteropCall` only accepts string arguments. For complex data, use a delimiter (e.g. `|`) and `split()` on the JavaScript side — this is significantly faster than `JSON.parse`.
</Aside>

- **Performance Critical:** Use for high-frequency calls and real-time updates
- **String Arguments:** Always pass data as strings - use a delimiter for complex structures
